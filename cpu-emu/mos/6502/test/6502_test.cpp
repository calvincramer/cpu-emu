...
// Xor
void xor_common_zero_page(CPU& cpu, u8 xorInst, u8 cycles, u8* offsetReg = nullptr, u8 offsetVal = 0) {
    // No wrap with offset
    cpu[RESET_LOC] = xorInst;
    cpu[RESET_LOC + 1] = 0x50;
    if (offsetReg != nullptr) { *offsetReg = offsetVal; }
    cpu.A = 0xF0;
...
void xor_common_absolute(CPU& cpu, u8 xorInst, u32 cycles, u8* offsetReg = nullptr, u8 offsetVal = 0) {
    // No page cross
    cpu[RESET_LOC] = xorInst;
    cpu[RESET_LOC + 1] = 0x68;
    cpu[RESET_LOC + 2] = 0x24;
...
TEST_F(EOR, ZeroPage)   { xor_common_zero_page(cpu, EOR_ZPG, 3); }
TEST_F(EOR, ZeroPageX)  { xor_common_zero_page(cpu, EOR_ZPX, 4, &cpu.X, 0x20); }
TEST_F(EOR, Absolute)   { xor_common_absolute (cpu, EOR_ABS, 4); }
TEST_F(EOR, AbsoluteX)  { xor_common_absolute (cpu, EOR_ABX, 4, &cpu.X, 0x10); }
TEST_F(EOR, AbsoluteY)  { xor_common_absolute (cpu, EOR_ABY, 4, &cpu.Y, 0x10);}
