...
u32 mos6502::CPU::execute(u32 p_numCycles) {
    ...
    while (numCycles > 0) {
        ...
        switch (currentInstr) {
            ...
            case AND_IMM : arithmetic(IMM, bitwise_and);                   break;
            case AND_ZPG : arithmetic(ZPG, bitwise_and);                   break;
            case AND_ZPX : arithmetic(ZPX, bitwise_and, X);                break;
            case AND_ABS : arithmetic(ABS, bitwise_and);                   break;
            case AND_ABX : arithmetic(ABX, bitwise_and, X);                break;
            case AND_ABY : arithmetic(ABY, bitwise_and, Y);                break;
            case AND_IDX : arithmetic(IDX, bitwise_and, X);                break;
            case AND_IDY : arithmetic(IDY, bitwise_and, Y);                break;
            case EOR_IMM : arithmetic(IMM, bitwise_eor);                   break;
            case EOR_ZPG : arithmetic(ZPG, bitwise_eor);                   break;
            case EOR_ZPX : arithmetic(ZPX, bitwise_eor, X);                break;
            case EOR_ABS : arithmetic(ABS, bitwise_eor);                   break;
            case EOR_ABX : arithmetic(ABX, bitwise_eor, X);                break;
            case EOR_ABY : arithmetic(ABY, bitwise_eor, Y);                break;
            case EOR_IDX : arithmetic(IDX, bitwise_eor, X);                break;
            case EOR_IDY : arithmetic(IDY, bitwise_eor, Y);                break;
            case ORA_IMM : arithmetic(IMM, bitwise_or);             break;
            case ORA_ZPG : arithmetic(ZPG, bitwise_or);             break;
            case ORA_ZPX : arithmetic(ZPX, bitwise_or, X);             break;
            case ORA_ABS : arithmetic(ABS, bitwise_or);             break;
            case ORA_ABX : arithmetic(ABX, bitwise_or, X);             break;
            case ORA_ABY : arithmetic(ABY, bitwise_or, Y);             break;
            case ORA_IDX : arithmetic(IDX, bitwise_or, X);             break;
            case ORA_IDY : arithmetic(IDY, bitwise_or, Y);             break;
            // Invalid instruction
            default: {
